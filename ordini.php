...
$ordini = $pdo->query("SELECT o.id_ordine, o.data, o.consegna_effettuata, i.citta, i.via, i.n_civico, c.nome_cognome, c.n_telefono FROM ordine o JOIN indirizzo_di_consegna i ON o.id_indirizzo = i.id_indirizzo JOIN registrazione r ON o.username_account = r.username_account JOIN cliente c ON r.email_cliente = c.email ORDER BY o.data DESC")->fetchAll();
$tutti_prodotti = $pdo->query("SELECT id_ordine, nome_prodotto, quantita FROM selezione")->fetchAll();
$prodotti_per_ordine = []; foreach ($tutti_prodotti as $p) { $prodotti_per_ordine[$p['id_ordine']][] = $p; }
?>
<!DOCTYPE html>
<html lang="it">
    ...
    <main class="content-area">
        ...
        <?php else: ?>
            <?php foreach ($ordini as $ordine): 
                $parti = explode(" ", $ordine['nome_cognome'], 2); $nome = $parti[0]??'-'; $cognome = $parti[1]??'-';
            ?>
            <div class="order-card">
                ...
                <div class="col-info">
                    <div class="split-row"><div class="box"><span class="label">Nome</span><span class="value"><?php echo htmlspecialchars($nome); ?></span></div><div class="box"><span class="label">Cognome</span><span class="value"><?php echo htmlspecialchars($cognome); ?></span></div></div>
                    <div class="box"><span class="label">Telefono</span><span class="value"><?php echo htmlspecialchars($ordine['n_telefono']??'-'); ?></span></div>
                    <div class="split-row"><div class="box"><span class="label">Data Ordine</span><span class="value"><?php echo date('d/m/y H:i', strtotime($ordine['data'])); ?></span></div><div class="box"><span class="label">Accettato</span><span class="status-indicator <?php echo $ordine['consegna_effettuata']?'status-G':'status-R'; ?>"><?php echo $ordine['consegna_effettuata']?'G':'R'; ?></span></div></div>
